...
bindersFreeαElem :  {F : Functor}(xs : List V)(e : μ F)
    → ∃ (λ e' →  ListNotOcurrBind  xs e'                                      ×
                 ((x : V)(x∈be' : x ∈b e') → notOcurrBindExcept x e' x∈be') ×
                 ∃ (λ bs → ListOcurrBind bs e × ((x : V)(x∈be : x ∈b e) → x ∈ bs))) 
\end{code}
%</bindersfreealphaelem>
...
lemma-bindersFreeα-∉b : {F : Functor}(xs : List V)(e : μ F)
  → ListNotOcurrBind {F} ((fv (bindersFreeα xs e)) ++ xs) (bindersFreeα xs e)
lemma-bindersFreeα-∉b xs e
  = subst  (λ ys → ListNotOcurrBind (ys ++ xs) (bindersFreeα xs e))
...
lemma-bindersFreeα-FV∉b : {F : Functor}(xs : List V)(e : μ F)
  → ListNotOcurrBind {F} (fv (bindersFreeα xs e)) (bindersFreeα xs e)
...
lemma-fih∧notOcurrBind⇛fihα |R|         P e          xs fih notOcurrR  = fih notOcurrR , notOcurrR
...
aux {F} P xs hiα e hi notBind
  = hiα e (lemma-fih∧notOcurrBind⇛fihα {F} F P e xs hi (λ c c∈xs → notOcurrBRinv (notBind c c∈xs)))
...
αProof : {F : Functor}(P : μ F → Set)(xs : List V)
  → αCompatiblePred P 
  → ((e : μ F) → ListNotOcurrBind xs e → ListNotOcurrBind (fv e) e → P e )  
  → ∀ e → P e
\end{code}
%</alphaproof>
...
... | e' , notBind | e'~e
  ...
  where
  nb1 : ListNotOcurrBind xs e'
  nb1 = lemma-binds++l notBind
  nb2 : ListNotOcurrBind (fv e') e'
  nb2 = subst (λ e → ListNotOcurrBind e e') (lemma~fv (σ e'~e)) (lemma-binds++r {xs} notBind)
\end{code}
